...
export class StoryService {
    ...
    static async getRandomStoryFromDate(lastId : number, ratio: number, days: number) : Promise<Story> {
        ...
        let pastDateUnixStartStamp = this.getPastUnixTimeStamp(pastDateStart, days);
        ...
        do {
            ...
            if (approximateStory.time && approximateStory.time < pastDateUnixStartStamp) {
                let newIdJump = Math.round((pastDateUnixStartStamp - Number(approximateStory.time)) / ratio);
            ...
            //If bigger than End Unix Time Stamp : Id is reduced
                ...
                let newIdJump = Math.round((Number(approximateStory.time) - pastDateUnixEndStamp) / ratio);
                idAttempt = idAttempt - newIdJump;
            ...
            console.log(!approximateStory.time || approximateStory.time < pastDateUnixStartStamp || 
                approximateStory.time > pastDateUnixEndStamp || approximateStory.type.toString().toLowerCase() !== "story");
    ...
    static async getTopWordsFromOldPost(daysAgo: number){
        ...
        const pastStory = await this.getRandomStoryFromDate(latestStories[0], IdUnixRatio, daysAgo);
        if(pastStory.title){
